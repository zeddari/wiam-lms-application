package com.wiam.lms.repository.search;

import co.elastic.clients.elasticsearch._types.query_dsl.QueryStringQuery;
import com.wiam.lms.domain.Part;
import com.wiam.lms.repository.PartRepository;
import java.util.stream.Stream;
import org.springframework.data.elasticsearch.client.elc.ElasticsearchTemplate;
import org.springframework.data.elasticsearch.client.elc.NativeQuery;
import org.springframework.data.elasticsearch.core.SearchHit;
import org.springframework.data.elasticsearch.core.query.Query;
import org.springframework.data.elasticsearch.repository.ElasticsearchRepository;
import org.springframework.scheduling.annotation.Async;

/**
 * Spring Data Elasticsearch repository for the {@link Part} entity.
 */
public interface PartSearchRepository extends ElasticsearchRepository<Part, Long>, PartSearchRepositoryInternal {}

interface PartSearchRepositoryInternal {
    Stream<Part> search(String query);

    Stream<Part> search(Query query);

    @Async
    void index(Part entity);

    @Async
    void deleteFromIndexById(Long id);
}

class PartSearchRepositoryInternalImpl implements PartSearchRepositoryInternal {

    private final ElasticsearchTemplate elasticsearchTemplate;
    private final PartRepository repository;

    PartSearchRepositoryInternalImpl(ElasticsearchTemplate elasticsearchTemplate, PartRepository repository) {
        this.elasticsearchTemplate = elasticsearchTemplate;
        this.repository = repository;
    }

    @Override
    public Stream<Part> search(String query) {
        NativeQuery nativeQuery = new NativeQuery(QueryStringQuery.of(qs -> qs.query(query))._toQuery());
        return search(nativeQuery);
    }

    @Override
    public Stream<Part> search(Query query) {
        return elasticsearchTemplate.search(query, Part.class).map(SearchHit::getContent).stream();
    }

    @Override
    public void index(Part entity) {
        repository.findById(entity.getId()).ifPresent(elasticsearchTemplate::save);
    }

    @Override
    public void deleteFromIndexById(Long id) {
        elasticsearchTemplate.delete(String.valueOf(id), Part.class);
    }
}
