...
/**
 * REST controller for managing {@link com.wiam.lms.domain.Professor}.
 */
@RestController
...
public class ProfessorResource {
    ...
    public ResponseEntity<Professor> createProfessor(@Valid @RequestBody Professor professor) throws URISyntaxException {
        ...
        professorSearchRepository.index(result);
    ...
    public ResponseEntity<Professor> partialUpdateProfessor(
        ...
        Optional<Professor> result = professorRepository
            ...
            .map(existingProfessor -> {
                ...
                if (professor.getMobileNumber() != null) {
                    existingProfessor.setMobileNumber(professor.getMobileNumber());
                }
                if (professor.getGender() != null) {
                    existingProfessor.setGender(professor.getGender());
                }
                if (professor.getAbout() != null) {
                    existingProfessor.setAbout(professor.getAbout());
                }
                if (professor.getImageLink() != null) {
                    existingProfessor.setImageLink(professor.getImageLink());
                }
                if (professor.getImageLinkContentType() != null) {
                    existingProfessor.setImageLinkContentType(professor.getImageLinkContentType());
                }
                if (professor.getCode() != null) {
                    existingProfessor.setCode(professor.getCode());
                }
                if (professor.getBirthdate() != null) {
                    existingProfessor.setBirthdate(professor.getBirthdate());
                }
                if (professor.getLastDegree() != null) {
                    existingProfessor.setLastDegree(professor.getLastDegree());
                }
            ...
            .map(savedProfessor -> {
                professorSearchRepository.index(savedProfessor);
    ...
    public ResponseEntity<Void> deleteProfessor(@PathVariable("id") Long id) {
        ...
        professorSearchRepository.deleteFromIndexById(id);
        return ResponseEntity
            .noContent()
    ...
    /**
     * {@code SEARCH  /professors/_search?query=:query} : search for the professor corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public List<Professor> searchProfessors(@RequestParam("query") String query) {
        log.debug("REST request to search Professors for query {}", query);
        try {
            return StreamSupport.stream(professorSearchRepository.search(query).spliterator(), false).toList();
