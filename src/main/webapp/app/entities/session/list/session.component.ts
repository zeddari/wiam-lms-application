...
import { EntityArrayResponseType, SessionService } from '../service/session.service';
...
export class SessionComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISession): number => this.sessionService.getSessionIdentifier(item);

  search(query: string): void {
    if (query && SessionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (SessionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.sessions = this.refineData(dataFromBody);
  }

  protected refineData(data: ISession[]): ISession[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ISession[] | null): ISession[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.sessionService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.sessionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
