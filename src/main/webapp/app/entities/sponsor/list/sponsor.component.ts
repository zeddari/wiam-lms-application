...
import { EntityArrayResponseType, SponsorService } from '../service/sponsor.service';
...
export class SponsorComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISponsor): number => this.sponsorService.getSponsorIdentifier(item);

  search(query: string): void {
    if (query && SponsorComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (SponsorComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.sponsors = this.refineData(dataFromBody);
  }

  protected refineData(data: ISponsor[]): ISponsor[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ISponsor[] | null): ISponsor[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.sponsorService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.sponsorService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
