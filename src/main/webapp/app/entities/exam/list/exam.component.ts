...
import { EntityArrayResponseType, ExamService } from '../service/exam.service';
...
export class ExamComponent implements OnInit {
  ...
  trackId = (_index: number, item: IExam): number => this.examService.getExamIdentifier(item);

  search(query: string): void {
    if (query && ExamComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ExamComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.exams = this.refineData(dataFromBody);
  }

  protected refineData(data: IExam[]): IExam[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IExam[] | null): IExam[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.examService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.examService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
