...
export class GroupComponent implements OnInit {
  ...
  trackId = (_index: number, item: IGroup): number => this.groupService.getGroupIdentifier(item);

  search(query: string): void {
    if (query && GroupComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (GroupComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.groups = this.refineData(dataFromBody);
  }

  protected refineData(data: IGroup[]): IGroup[] {
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.groupService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
